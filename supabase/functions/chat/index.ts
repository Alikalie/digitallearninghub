...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch admin-added custom knowledge and knowledge files
    let customKnowledge = "";
    ...
    try {
      ...
      // Load knowledge files metadata
      const { data: filesData } = await sb.from("admin_settings").select("value").eq("key", "bot_knowledge_files").maybeSingle();
      if (filesData?.value && Array.isArray(filesData.value)) {
        const files = filesData.value as Array<{ name: string; url: string; type: string }>;
        if (files.length > 0) {
          // For text-based files, try to fetch content
            ...
            if (isTextFile) {
              try {
                ...
                if (resp.ok) {
                  ...
                  const truncated = text.length > 5000 ? text.substring(0, 5000) + "... [truncated]" : text;
                  fileDescriptions.push(`FILE: ${file.name}\nCONTENT:\n${truncated}`);
                }
              } catch {
                fileDescriptions.push(`FILE: ${file.name} (available at ${file.url})`);
              }
            } else {
              fileDescriptions.push(`REFERENCE FILE: ${file.name} (type: ${file.type}, available at: ${file.url}). If asked about this file, let the user know it's available and describe what you can infer from its name and type.`);
            }
          }
    ...
    const fullSystemPrompt = SYSTEM_PROMPT + customKnowledge + knowledgeFilesContext + coursePrompt;
